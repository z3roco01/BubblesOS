...
void vgaDrawChar(uint32_t x, uint32_t y, char c) {
    uint8_t* charData = font8x8[c];

    for(uint32_t cy = 0; cy < FONT_WIDHT; ++cy) {
        for(uint32_t cx = 0; cx < FONT_HEIGHT; ++cx) {
            vgaPutPix(x+cx, y+cy, charData[cy] & (1 << cx) ? FONT_FG : FONT_BG);
        }
    }
}
